...
namespace Client
{
    ...
    internal enum Command
    {
        ...
        PacketSize,
        //Get the size in bytes of the next incoming packet so we can size our receiving packet accordingly. Used for receiving the DataInfo packets.
    ...
    public partial class ClientForm : Form
    {
        ...
        private readonly WaveFileReader blipReader;
        private readonly DirectSoundOut blipPlayer = new DirectSoundOut();
        private WaveFileReader wr;
        private readonly DirectSoundOut sfxPlayer = new DirectSoundOut();
        private DmoMp3Decoder musicReader;
        private readonly DirectSoundOut musicPlayer = new DirectSoundOut();
        ...
        private readonly SolidBrush reportsBackgroundBrushGreen = new SolidBrush(Color.LightGreen);
        private readonly SolidBrush reportsBackgroundBrushRed = new SolidBrush(Color.Red);
        ...
        private void ClientForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                if (Program.debug)
                    MessageBox.Show(ex.Message + ".\r\n" + ex.StackTrace, "AODXClient: " + strName, MessageBoxButtons.OK,
        ...
        private void musicList_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            {
                var text = musicList.Items[index].ToString();
                var g = e.Graphics;
        ...
        private void OnReceive(IAsyncResult ar)
        {
            try
            {
                ...
                if (byteData[0] == 8)
                {
                    ...
                    using (var ms = new MemoryStream(data.dataBytes))
                    {
                        evi.icon = Image.FromStream(ms, false, true);
                    }

                    var found = false;
                    foreach (var item in eviList)
                    {
                ...
                else
                {
                    ...
                    switch (msgReceived.cmdCommand)
                    {
                        ...
                        case Command.Present:
                            ...
                            if (msgReceived.callout <= 3)
                            {
                                ...
                                if (iniParser.GetSoundName(msgReceived.strName, msgReceived.anim) != "1" &&
                                    iniParser.GetSoundTime(msgReceived.strName, msgReceived.anim) > 0 &
                                    File.Exists("base/sounds/general/" + iniParser.GetSoundName(latestMsg.strName, latestMsg.anim) + ".wav"))
                                {
                                    sfxPlayer.Stop();
                                    wr =
                                        new WaveFileReader("base/sounds/general/" + iniParser.GetSoundName(latestMsg.strName, latestMsg.anim) + ".wav");
                                    sfxPlayer.Initialize(wr);
                                ...
                                else //if there is a pre-animation
                                {
                                    ...
                                    setCharSprite("base/characters/" + msgReceived.strName + "/" +
                    ...
                    if (msgReceived.strMessage != null & msgReceived.cmdCommand == Command.Message |
                        msgReceived.cmdCommand == Command.Login | msgReceived.cmdCommand == Command.Logout)
                    {
                        if (msgReceived.callout <= 3)
            ...
            for (var i = 0; i < 3; i++)
            {
                if (TextRenderer.MeasureText(msgText, displayMsg1.Font).Width > 240)
                {
                    ...
                    for (var x = 0; x < parts.Length; x++)
                    {
                        ...
                        {
                            for (var y = 0; y < parts[x].Length; y++)
                            {
                    ...
                    catch (Exception ex)
                    {
                        if (Program.debug)
                            MessageBox.Show("Unable to send message to the server.\r\n" + ex.Message + ".\r\n" + ex.StackTrace,
                                "AODXClient: " + strName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
        }
        ...
        private void dispTextRedraw_Tick(object sender, EventArgs e)
        {
            if (redraw)
            {
                for (var x = 0; x < textToDisp.Length; x++)
                {
                ...
                {
                    blipPlayer.Stop();
                    setCharSprite("base/characters/" + latestMsg.strName + "/(a)" +
                                  iniParser.GetAnim(latestMsg.strName, latestMsg.anim) + ".gif");
                    redraw = false;
            ...
            try
            {
                if ((soundTime > 4 && curSoundTime < (soundTime - 4)) | (soundTime > 0 & soundTime < 4 & curSoundTime < soundTime))
                    curSoundTime++;
                         ...
                         (soundTime > 0 & soundTime < 4 & curSoundTime == soundTime))
                {
                ...
                if (preAnimTime > 0 && curPreAnim != null)
                {
                    ...
                    {
                        //if (charLayerImage.FrameDimensionsList.Length > curPreAnimTime)
                        //charLayerImage.SelectActiveFrame(new System.Drawing.Imaging.FrameDimension(charLayerImage.FrameDimensionsList[curPreAnimTime]), curPreAnimTime);
                        curPreAnimTime++;
                    }
                    else
                    {
                        ...
                        setCharSprite("base/characters/" + latestMsg.strName + "/(b)" +
                                      iniParser.GetAnim(latestMsg.strName, latestMsg.anim) + ".gif");
                ...
                /* else
				{
					...
					if (latestMsg != null)
					{
						charLayerImage = Image.FromFile("base/characters/" + latestMsg.strName + "/(b)" + latestMsg.anim + ".gif");
						prepWriteDispBoxes(latestMsg, latestMsg.textColor);
					}
				} */
            }
        ...
        private void musicList_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (musicList.Items.Count > 0 && musicList.SelectedItem != null && (string)musicList.SelectedItem != "")
            {
                var msgToSend = new Data
                {
                    cmdCommand = Command.ChangeMusic,
                    strName = strName,
                    strMessage = (string)musicList.Items[musicList.SelectedIndex]
                };
                var msg = msgToSend.ToByte();

                clientSocket.BeginSend(msg, 0, msg.Length, SocketFlags.None, OnSend, null);
        ...
        {
            if (btn_defminus.Visible)
            {
                if (defHealth > 0)
                {
                    var msg = new Data
                    {
                        cmdCommand = Command.ChangeHealth,
                        strName = "def",
                        strMessage = "-1"
                    };
                    var byteMsg = msg.ToByte();
                    clientSocket.BeginSend(byteMsg, 0, byteMsg.Length, SocketFlags.None, OnSend, null);
                }
            }
        }
        ...
        {
            if (btn_testimony.Visible)
            {
                var calloutMsg = new Data
                {
                    callout = 4,
                    cmdCommand = Command.Message
                };

                var msg = calloutMsg.ToByte();
                clientSocket.BeginSend(msg, 0, msg.Length, SocketFlags.None, OnSend, null);
            }
        }
        ...
        {
            if (btn_crossexamination.Visible)
            {
                var calloutMsg = new Data
                {
                    callout = 5,
        ...
        private void emoButton_Click(object sender, EventArgs e)
        {
            ...
            {
                var button = sender as IndexButton;
        ...
        private void showEvidenceInfo()
        {
            ...
            var infoBox = new PictureBox
            {
                Name = "infoBox",
                Parent = courtRecordPB,
                Location = new Point(30, 77),
                Size = new Size(440, 222),
                Image = Image.FromFile("base/misc/inventory_disp.png")
            };
            ...
            var icon = new PictureBox
            {
                ...
                Size = new Size(70, 70),
                Image = eviList[selectedEvidence].icon
            };

            var eviName = new Label
            {
                Name = "eviName",
                Parent = infoBox,
                ...
                AutoSize = false,
                TextAlign = ContentAlignment.MiddleCenter,
                Size = new Size(338, 17),
                BackColor = Color.Transparent,
                ForeColor = Color.DarkOrange,
                Font = new Font(fonts.Families[0], 12.0f),
                Text = eviList[selectedEvidence].name
            };

            var eviNote = new Label
            {
                Name = "eviNote",
                Parent = infoBox,
                Location = new Point(95, 31),
                AutoSize = false,
            ...
            var eviDesc = new Label
            {
                ...
                AutoSize = false,
                TextAlign = ContentAlignment.TopLeft,
                Size = new Size(434, 112),
                BackColor = Color.Transparent,
                ForeColor = Color.White,
                Font = new Font(fonts.Families[0], 12.0f),
                Text = eviList[selectedEvidence].desc
            };
        ...
        private void btn_back_PerformClick()
        {
            ...
            foreach (Control ctrl in courtRecordPB.Controls)
            {
                if (ctrl.Name == "infoBox" | ctrl.Name == "icon" | ctrl.Name == "eviName" | ctrl.Name == "eviDesc" |
                    ctrl.Name == "eviNote")
                    ctrl.Dispose();
        ...
        private void btn_edit_Click(object sender, EventArgs e)
        {
            if (readyToPresent && btn_edit.Visible & btn_edit.Enabled)
            {
                ...
                {
                    var index = 0;
                    foreach (var item in editor.evidence)
                    {
                        if (item.index == selectedEvidence)
                        {
                            ...
                            var msgToSend = new EviData(item.name, item.desc, item.note, item.index);
    ...
    internal class Data
    {
        ...
        public Data(byte[] data)
        {
            ...
            var textColorLen = BitConverter.ToInt32(data, 13);

            //The next four store the length of the message
            var msgLen = BitConverter.ToInt32(data, 17);
    ...
    internal class EviData
    {
        ...
        public EviData(byte[] data)
        {
            ...
            //The next four store the length of the name
            var nameLen = BitConverter.ToInt32(data, 8);

            var descLen = BitConverter.ToInt32(data, 12);

            var noteLen = BitConverter.ToInt32(data, 16);
