...
namespace Server
{
    public static class iniParser
    {
        ...
        public static List<string> GetCharList()
        {
            ...
            {
                MessageBox.Show("Unable to load characters: Folder does not exist!", "Error");
                return chars;
            }

            foreach (string dir in Directory.EnumerateDirectories(dirName).Where(dir => File.Exists(dir + "/char.ini")))
            {
            ...
            if (chars.Count == 0)
            {
                MessageBox.Show("Unable to load characters: No characters found!", "Error");
            }
            return chars;
        }
        ...
        public static List<string> GetMusicList()
        {
            ...
            if (Directory.Exists(dirName))
            {
                songs.AddRange(Directory.EnumerateFiles(dirName).Select(file => file.Substring(18)));
                return songs;
            }
            return null;
        }
        ...
        public static List<Evidence> GetEvidenceData()
        {
            ...
            if (!Directory.Exists(dirName))
            {
                MessageBox.Show("Unable to load evidence: Folder does not exist!", "Error");
                return evidence;
            }

            foreach (string dir in Directory.EnumerateDirectories(dirName).Where(dir => dir.Substring(11) == GetServerInfo().Split('|')[9]))
            {
                foreach (string file in Directory.EnumerateFiles(dir))
                {
                    using (var fs = new FileStream(file, FileMode.Open))
                    {
                        Evidence evi = new Evidence {filename = file.Split('.').First()};
            ...
            if (evidence.Count == 0)
            {
                MessageBox.Show("Unable to load evidence: No evidence found!", "Error");
            }
            return evidence;
        }
