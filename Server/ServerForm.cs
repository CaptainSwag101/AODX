...
namespace Server
{
    ...
    public partial class ServerForm : Form
    {
        ...
        private void sendServerInfo(Socket socketToUse)
        {
            try
            {
                List<byte> msgToSend = new List<byte> {101};
                ...
                if (!isClosing)
                {
                    ...
                    switch (msgReceived.cmdCommand)
                    {
                        ...
                        case Command.PacketSize:
                            ...
                            // List<string> charsInUse = (from cName in allChars where clientList != null && clientList.Count > 0 from ClientInfo client in clientList where client.strName == cName select cName).ToList();
                            foreach (string cName in allChars.Where(cName => clientList != null && clientList.Count > 0))
                            {
                                charsInUse.AddRange(from ClientInfo client in clientList where client.strName == cName select cName);
                            }
        ...
        private void ServerForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                List<byte> msgToSend = new List<byte> { 103 };
                byte[] message = msgToSend.ToArray();
            ...
            {
                if (Program.debug)
                    MessageBox.Show(ex.Message + "\r\n" + ex.StackTrace, "AODXServer", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
            ...
            if (lstUsers.SelectedItem != null)
            {
                ...
                if (!silent)
                {
                    Data msgToSend = new Data
                    {
                        ...
                        strMessage = "<<<" + username + " has been kicked from the courtroom>>>"
                    };
        ...
        private void banUser()
        {
            if (lstUsers.SelectedItem != null)
            {
                ...
                {
                    cmdCommand = Command.Logout,
                    strMessage = "<<<" + ((string) lstUsers.SelectedItem).Split(new string[] {" - "}, StringSplitOptions.None)[0] + " has been banned from the courtroom>>>"
                };
                byte[] msg = msgToSend.ToByte();
